...
Utilise V3 de tous les modules
...
def safe_rerun() -> None:
    """Appelle st.rerun() quand l'application est lancée par Streamlit;
    ignore proprement l'appel quand le script est exécuté en dehors de Streamlit.
    """
    try:
        st.rerun()
    except Exception:
...
st.sidebar.title("Dashboard Projet S3")
...
with st.sidebar.expander("Ajouter des zones"):
    ...
    if st.session_state.graphiques.lines != []:
        ...
        titre_nouvelle_zone = st.text_input("Entrez le titre de la nouvelle zone")
        ...
        type_graphique = st.radio(
            "Choisissez le type de graphique à implémenter", options
        )
        if st.button("Ajouter une zone"):
            ...
            if not type_graphique:
                st.error("Aucun type de graphique sélectionné")
            else:
...
with st.sidebar.expander("Retirer des zones"):
    ...
    ligne_selectionnée = st.selectbox(
        "Sélectionnez la ligne contenant la zone à supprimer", lines_names
    )
    ...
    if nom_ligne_modifiée:
        ...
        if nom_area_modifié and nom_ligne_modifiée:
            ...
            if st.session_state.données.data is not None and len(st.session_state.données.data.columns) > 0:
                ...
                colonnes_deja_séléctionnées = st.session_state.graphiques.get_area_ploted_columns(
                    nom_ligne_modifiée, nom_area_modifié
                )
                colonnes_affichées = st.multiselect(
                    "Choisissez les colonnes utilisées dans le graphique",
                    colonnes_données,
                ...
                if colonnes_affichées:
                    ...
                    st.session_state.graphiques.set_datas(nom_ligne_modifiée, nom_area_modifié, données_affichées)  # type: ignore

                    st.subheader("Choix de l'axe d'abscisse")
                    colonne_abscisse = st.selectbox(
                        "Sélectionnez la colonne à mettre en axe des abscisses",
                        [""] + colonnes_affichées,
                    )
                    if colonne_abscisse:
...
page = st.session_state.get("page", "Données")
